...
class assign_grading_table extends table_sql implements renderable {
	...
	$fields .= 'u.timecreated as timecreated, ';
        $fields .= 's.status as status, ';
	$fields .= 's.id as submissionid, ';
	$fields .= 'pa.projectid as projectid, ';
	$fields .= 'p.name as projectname, ';
        ...
        $from .= ' LEFT JOIN moodle.plass_projectsubmission pa
				ON u.id = pa.userid 
                               AND pa.assignid = :assignmentid4 ';

        $from .= ' LEFT JOIN moodle.plass_project p
        ...
        $columns[] = 'project';
        $headers[] = get_string('project', 'assign');
    ...
    public function col_project(stdClass $row) {
        $o = '';
        $link = '';
        $separator = $this->output->spacer(array(), true);
        $grade = '';
        
	$token = hash('sha256', $row->timecreated);
	//$token = base64_encode($token);
        $urlparams = array('id' => $row->id, 'token' => $token);
        $url = new moodle_url('http://210.94.194.63:5110', $urlparams);
        $link = '<a href="' . $url . '" class="btn btn-primary" target="_blank">' . get_string('project', 'assign') . '</a>';
	$grade .= $link . $separator;

	if($row->projectname)
		$grade .= $row->projectname;
	else
		$grade .= get_string('noproject', 'assign');
