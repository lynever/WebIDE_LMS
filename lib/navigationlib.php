...
class navigation_node implements renderable {
    ...
    public static function create($text, $action=null, $type=self::TYPE_CUSTOM,
            $shorttext=null, $key=null, pix_icon $icon=null, $target=null) {
     ...
     * @return navigation_node
     */
    public function add($text, $action=null, $type=self::TYPE_CUSTOM, $shorttext=null, $key=null, pix_icon $icon=null, $target=null) {
        // Create child node
        $childnode = self::create($text, $action, $type, $shorttext, $key, $icon, $target);
...
class global_navigation extends navigation_node {
	...
	if($navoptions->project) {
	    ...
	    $projecturl = new moodle_url('http://210.94.194.63:5110', $params);
	...
	if($navoptions->problembank) {
	    ...
	    $projecturl = new moodle_url('');
	    $node = $coursenode->add(get_string('problembank'), $projecturl,
		self::TYPE_CUSTOM, null, 'problembank', new pix_icon('i/privatefiles', ''));
	    $node->showinflatnavigation = true;
	}
...
class settings_navigation extends navigation_node {
     ...
     */
    public function add($text, $url=null, $type=null, $shorttext=null, $key=null, pix_icon $icon=null, $target=null) {
        $node = parent::add($text, $url, $type, $shorttext, $key, $icon, $target);
